...
# changed implementation to accomodate empty files (relax assert statement)
def read_object_property_file(path, delimiter=',', cols_to_read=[0, 1],
                              keep_header=False):
    f = open(path, 'r')
    reader = csv.reader(f, delimiter=delimiter)
    # add reshaping in case of empty file.
    ...
    data = [[entry[col] for entry in data_full[start_position:]] for col in cols_to_read]
